...
from dataset import mnist, fashionmnist, cifar10, cifar100
...
if dataset == 'mnist':
    ...
    train_loader, test_loader, demo_loader = fashionmnist(train_batch, test_batch, amp=False)
    dim_in = 28 * 28
    dim_out = 10
    ...
    train_loader, test_loader, demo_loader = cifar100(train_batch, test_batch, amp=amp)
    dim_in = 32 * 32 * 3
    dim_out = 10
...
if dataset == 'mnist':
    ...
    if dataset == 'fashionmnist':
        conv_net = True
        conv_settings = ((1, 20, 5, 2, 0), (20, 50, 5, 2, 0))
        dim_s = np.array([800, 500])
        Cs_conv = np.ones(len(conv_settings)) * 0.2
        bn_s = [False] * len(conv_settings)
        pool_s = [False, False] * (len(conv_settings) // 2)
        Cs_list = np.array([1]) * 0.2
        C_1 = torch.tensor(0.2)
    ...
    if args.heaviside:
        if args.batch_cov:
            net = CNN_HeavisideNet(dim_s=dim_s, dim_out=dim_out, conv_settings=conv_settings, use_bias=use_bias,
        ...
        if args.batch_cov:
            net = CNN_ReluNet(dim_s=dim_s, dim_out=dim_out, conv_settings=conv_settings, use_bias=use_bias,
                                    pooltype=pooltype, bn_s=bn_s, pool_s=pool_s).to(device)
        else:
            net = CNN_ReluNet2(dim_s=dim_s, dim_out=dim_out, conv_settings=conv_settings, use_bias=use_bias,
                                    pooltype=pooltype, bn_s=bn_s, pool_s=pool_s).to(device)
